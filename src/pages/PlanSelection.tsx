...
const PlanSelection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      try {
        ...
        setError(e.message || 'プラン取得に失敗しました');
        setLoading(false);
      }
    }, 800);
  }, [t]);
  ...
  if (error) return <div className="text-center text-red-600 py-8">{error}<button onClick={() => window.location.reload()} className="ml-4 px-4 py-2 bg-blue-500 text-white rounded">再試行</button></div>;
